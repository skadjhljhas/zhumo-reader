...
import { AI_IPC, type AiApi, type AiEvent } from '../shared/ai-types'

const ai: AiApi = {
  activity: () => ipcRenderer.invoke(AI_IPC.activity),
  getProfiles: () => ipcRenderer.invoke(AI_IPC.profiles),
  saveProfile: (lane, profile) => ipcRenderer.invoke(AI_IPC.save, lane, profile),
  start: (request) => ipcRenderer.invoke(AI_IPC.start, request),
  cancel: (id) => ipcRenderer.invoke(AI_IPC.cancel, id),
  onEvent: (callback) => {
    const listener = (_event: Electron.IpcRendererEvent, data: AiEvent): void => callback(data)
    ipcRenderer.on(AI_IPC.event, listener)
    return () => {
      ipcRenderer.removeListener(AI_IPC.event, listener)
    }
  }
}
...
ipcRenderer.on('app:request-close', () => {
  ...
  // Before Vue mounts there is no editable document; still finish the close handshake.
  if (closeCallbacks.size === 0) void ipcRenderer.invoke(IPC.CloseConfirmed)
})
...
const api: ZhuMoApi & FlushApi = {
  ...
  readFont: (id) => ipcRenderer.invoke(IPC.ReadFont, id),
  manuscriptLocation: () => ipcRenderer.invoke(IPC.ManuscriptLocation),
  chooseManuscriptLocation: () => ipcRenderer.invoke(IPC.ChooseManuscriptLocation),
  revealManuscriptLocation: () => ipcRenderer.invoke(IPC.RevealManuscriptLocation),
  saveBook: (path, content, expected) => ipcRenderer.invoke(IPC.SaveBook, path, content, expected),
  saveBookAs: (name, content) => ipcRenderer.invoke(IPC.SaveBookAs, name, content),
  setDocumentDirty: async (dirty) => {
    ipcRenderer.send(IPC.DocumentDirty, dirty)
  },
