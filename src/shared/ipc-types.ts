...
export interface Settings {
  ...
  /** 脚注层级上限，1–6，默认 4 */
  noteLevelCap: number
  selectionNote: boolean
  selectionExplain: boolean
  automaticSyntax: boolean
  annotationMode: 'follow' | 'document'
  /** 自动文本标注前连续可见的秒数，1–15，默认 5。 */
  automaticSyntaxWaitSeconds: number
...
export interface BookPayload {
  ...
  epub?: import('./epub-types').EpubBook
  path: string
  content: string
  title: string
}

/** window.api 的完整形状（由 preload 的 contextBridge 暴露，全部为 invoke 请求-响应式调用） */
export interface ZhuMoApi {
  exportPdf(request: PdfExportRequest): Promise<{ path: string; missingImages: number } | null>
  listFonts(): Promise<ImportedFont[]>
  importFont(): Promise<FontPayload | null>
  readFont(id: string): Promise<Uint8Array>
  manuscriptLocation(): Promise<{ path: string; available: boolean; error?: string }>
  chooseManuscriptLocation(): Promise<{ path: string; available: boolean; error?: string } | null>
  revealManuscriptLocation(): Promise<void>
  /** 系统打开对话框（.md/.markdown/.txt）；取消返回 null，成功即更新最近列表 */
  openBookDialog(): Promise<BookPayload | null>
  /** utf8 读取图书并记入最近列表（T22，与 openBookDialog 对齐）；不存在 / 不可读 / 超 50MB 会 reject（中文错误信息） */
  readBook(path: string): Promise<BookPayload>
  /** 保存源文；expected 是上次读取的完整内容，用于防止覆盖外部修改。 */
  saveBook(path: string, content: string, expected: string): Promise<BookPayload>
  saveBookAs(suggestedName: string, content: string): Promise<BookPayload | null>
  ...
  getDisplayInfo?(): Promise<{ refreshRate: number; scaleFactor: number }>
  /** 最近打开列表（最近优先，上限 12 条） */
  getRecentBooks(): Promise<RecentBook[]>
  /** 从最近列表移除指定路径 */
  removeRecent(path: string): Promise<void>
  /** 由 File 对象对应的磁盘路径（拖放开书用；必须在 preload 中执行） */
...
export const IPC = {
  ExportPdf: 'book:export-pdf',
  ListFonts: 'fonts:list',
  ImportFont: 'fonts:import',
  ReadFont: 'fonts:read',
  ManuscriptLocation: 'library:location',
  ChooseManuscriptLocation: 'library:choose-location',
  RevealManuscriptLocation: 'library:reveal-location',
  OpenBookDialog: 'book:open-dialog',
  ReadBook: 'book:read',
  SaveBook: 'book:save',
  SaveBookAs: 'book:save-as',
  DocumentDirty: 'document:dirty',
  CloseConfirmed: 'document:close-confirmed',
  SaveProgress: 'progress:save',
  GetProgress: 'progress:get',
  SaveSettings: 'settings:save',
  GetSettings: 'settings:get',
  GetDisplayInfo: 'display:info',
...
export const DEFAULT_SETTINGS: Settings = {
  ...
  noteLevelCap: 4,
  selectionNote: false,
  selectionExplain: false,
  automaticSyntax: true,
  annotationMode: 'follow',
  automaticSyntaxWaitSeconds: 5,
  selectionCopy: false
}
...
export type NumericSettingsKey =
  'fontSize' | 'lineHeight' | 'contentWidth' | 'noteLevelCap' | 'automaticSyntaxWaitSeconds'
...
export const SETTINGS_LIMITS: Record<NumericSettingsKey, NumericSettingLimit> = {
  ...
  automaticSyntaxWaitSeconds: { min: 1, max: 15, step: 1 }
}
...
export function clampSettings(input: unknown): Settings {
  ...
  return {
    lightRange: raw.lightRange === 'hdr' ? 'hdr' : 'sdr',
    uiFont: fontId(raw.uiFont),
    bodyFont: fontId(raw.bodyFont),
    noteFont: fontId(raw.noteFont),
    selectionNote: typeof raw.selectionNote === 'boolean' ? raw.selectionNote : false,
    selectionExplain: typeof raw.selectionExplain === 'boolean' ? raw.selectionExplain : false,
    automaticSyntax: typeof raw.automaticSyntax === 'boolean' ? raw.automaticSyntax : true,
    annotationMode: raw.annotationMode === 'document' ? 'document' : 'follow',
    automaticSyntaxWaitSeconds: clampNumber(
      raw.automaticSyntaxWaitSeconds,
      SETTINGS_LIMITS.automaticSyntaxWaitSeconds,
      DEFAULT_SETTINGS.automaticSyntaxWaitSeconds,
      true
    ),
    selectionCopy: typeof raw.selectionCopy === 'boolean' ? raw.selectionCopy : false,
