...
export interface NoteBodySpot {
  ...
  /** 所在章标题（「见于·〈章名〉」展示用） */
  sectionTitle: string
  /** Most recent H1–H3 entry; independent of the internal rendering chunk. */
  headingId?: string
}
...
export interface NoteRecord {
  selectionRange?: import('./note-range').NoteRange
  id: string
  label: string
...
export interface ParsedBook {
  ...
  /** Sparse source landmarks for synchronized reading/editing (LF offsets). */
