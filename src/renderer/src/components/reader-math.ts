...
export const READER_TUNING = {
  ...
  /** 侧栏跟随平滑（glide，T24）：临界阻尼弹簧角频率（rad/s）。约 5.8/ω≈290ms
   *  收敛到 2%；常规跟随距离（一两张卡，240–300px）95% 落位约 240ms，
   *  既在 180–320ms 的手感区间内，尾段又只是亚像素微调 */
  followGlideOmega: 20,
  /** glide 巡航速度上限（px/ms）：远距目标不因弹簧峰值速度变成一闪而过 */
  followGlideMaxSpeed: 4.5,
  /** glide 单次时长上限（ms）：超远目标按 距离/maxMs 抬高巡航速度，保证有界落位 */
  followGlideMaxMs: 700,
  /** glide 收敛阈值（px）：位置误差小于此值即吸附（scrollTop 按像素量化） */
  followGlideSnapPx: 0.5,
  /** glide 单帧时间步上限（ms，T25）：约 1.5 帧。布局 churn（停靠进出、
   *  虚拟列表重量测）会把某一帧拉长到 50ms，弹簧按真实 dt 积分就在这一帧
   *  走掉 72px（停靠）乃至 288px（跟随）——物理正确，视觉上是一次硬跳。
   *  夹住时间步：长帧只让动画变慢，不让它跳。配合 hoverGlideMaxSpeed，
   *  停靠通道单帧位移上界 ≈ 2.2 × 26 ≈ 57px */
  glideMaxFrameMs: 26,
  /** glide 单帧屏幕位移硬上界（px，T25）：按巡航速度反解时间步
   *  （dt ≤ maxStep/cap），把上界变成与目标距离无关的结构性质。时间步夹取
   *  只管长帧，管不住远距目标：12384px 的极端停靠会把 cap 抬到 5.16px/ms，
   *  26ms 的长帧实测仍是 131px/帧。取 100px——低于验收线 120px 且留 17%
   *  余量；对 60Hz 下的跟随（4.5px/ms × 16.7ms = 75px）与常规停靠
   *  （2.2px/ms × 16.7ms = 37px）都不生效，T24 手感不变。代价只在远距停靠：
   *  同样行程要走更多帧，即「更慢换更平滑」（用户已授权） */
  glideMaxStepPx: 100,
  /** 悬停停靠对齐的平滑（T25）：临界阻尼角频率（rad/s），比跟随更柔。
   *  约 5.8/ω≈450ms 收敛到 2%，首帧只吃掉约 2% 误差——旧的一阶缓动
   *  τ≈85ms 首帧吃掉 17% 误差，170px 的停靠首帧瞬移 29px，远距停靠
   *  （首/尾注要让出整屏留白）实测单帧 2194px */
  hoverGlideOmega: 13,
  /** 停靠 glide 巡航速度上限（px/ms）：60Hz 下约 37px/帧，常规停靠距离
   *  （一两张卡）由弹簧自然收敛，不触及此上限 */
  hoverGlideMaxSpeed: 2.2,
  /** 停靠 glide 单次时长上限（ms）：超远停靠按 距离/maxMs 抬高巡航速度，
   *  保证有界落位（虚拟列表里从第一注停到最后一注也应在数秒内走完） */
  hoverGlideMaxMs: 2400,
...
/**
 * 侧栏跟随平滑插值（glide，T24）的调参视图：READER_TUNING 中四个 followGlide*
 * 常量的结构体形式，NotesSidebar 的 rAF 循环与单测共用。
 */
...
/** glide 一帧结果：亚像素位置、速度（px/ms）与是否已收敛 */
...
/** glide 巡航速度上限（px/ms）：常数下限与「距离 / 单次时长上限」取大，
 *  保证任意距离都在有限时间内落位（超远目标退化为匀速巡航而非瞬移）。 */
export function glideSpeedCap(distancePx: number, tuning: GlideTuning): number {
  return Math.max(tuning.maxSpeed, Math.abs(distancePx) / Math.max(1, tuning.maxMs))
}

/**
 * 侧栏跟随平滑插值一帧（glide，T24）：临界阻尼弹簧的闭式积分（帧率无关、
 *  无条件稳定）：x(t) = (x0 + (v0 + ωx0)t)e^{−ωt}，v(t) = (v0 − ω(v0 + ωx0)t)e^{−ωt}。
 *  两条限幅保证「可打断、不叠加、不过冲、不闪烁」：朝目标方向的速度超过
 *  ω|x0|（临界阻尼的无过冲上界）时压回——改目标不产生回摆；速度超过
 *  glideSpeedCap 时切匀速巡航段——远距目标不在首帧瞬移。新目标只替换 target，
 *  位置与速度连续；误差小于 snapPx 吸附到目标。纯函数，可单测。
 */
export function glideFrame(
  ...
  if (Math.abs(x0) <= tuning.snapPx) return { position: target, velocity: 0, settled: true }
  // 帧长异常（0/负/NaN，如后台标签页 rAF 停摆）退化为「本帧不动」，不产生 NaN 位置
  const dt = Number.isFinite(dtMs) ? Math.min(64, Math.max(0, dtMs)) / 1000 : 0
  const w = tuning.omega
  let v0 = velocity * 1000
  // 无过冲上界：朝目标方向的速度超过 ω|x0| 即压回（改目标不回摆）
  if (x0 * v0 < 0 && Math.abs(v0) > w * Math.abs(x0)) v0 = -w * x0
  const cap = glideSpeedCap(x0, tuning) * 1000
  const k = (v0 + w * x0) * dt
  const decay = Math.exp(-w * dt)
  const v1 = (v0 - w * k) * decay
  if (Math.abs(v1) > cap) {
    const vc = Math.sign(v1) * cap // px/s
    // 匀速巡航段：位移 = vc × dt（vc 为 px/s、dt 为 s）；速度以 px/ms 回传
    return { position: position + vc * dt, velocity: vc / 1000, settled: false }
  }
  return { position: target + (x0 + k) * decay, velocity: v1 / 1000, settled: false }
}

/**
 * scrollIntoView({ block:'nearest' }) 的等价位移（T24 平滑化）：视口相对坐标下，
 *  卡顶在视口上方 → 对齐视口顶；卡底在视口下方 → 对齐视口底；其余不动。
 *  纯函数，可单测；NotesSidebar 据此把 instant 定位换成 glide 目标。
 */
export function nearestScrollDelta(
  ...
  if (cardTop < 0) return cardTop
  if (cardBottom > viewHeight) return cardBottom - viewHeight
  return 0
}
