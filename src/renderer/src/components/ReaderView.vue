...
import type { NoteBodySpot, ParsedBook, Section, TocItem } from '../../../shared/types'
import { bookState } from '../composables/useBook'
import { settings } from '../composables/useSettings'
import DocumentOverture from './DocumentOverture.vue'
import { documentSession } from '../composables/documentSession'
import { studio } from '../composables/useStudio'
import { openNotePeek, closeNotePeek } from '../composables/notePeek'
...
import {
  ...
  documentPosition,
  bookPositionIndex
} from '../composables/documentPosition'
import { sourcePlace, type SourcePlace } from '../composables/sourcePosition'
import {
  nextFrames,
  pinFollowFocus,
  readerState,
...
const precedingHeadings = computed(() => {
  const positions = new Map(ownSections.value.map((section, index) => [section.id, index]))
  const entries = (ownBook.value?.toc ?? []).filter((item) => positions.has(item.sectionId))
  const result = new Map<string, TocItem | undefined>()
  let cursor = 0,
    previous: TocItem | undefined
  ownSections.value.forEach((section, index) => {
    while (cursor < entries.length && positions.get(entries[cursor].sectionId)! < index)
      previous = entries[cursor++]
    result.set(section.id, previous)
  })
  return result
})
...
function contentStart(): number {
  const root = scrollEl.value
  const first = frameEls.get(ownSections.value[0]?.id)
  if (!root || !first || root.clientHeight === 0) return 300
  return first.getBoundingClientRect().top - root.getBoundingClientRect().top + root.scrollTop
}

function prefixTops(): number[] {
  const heights = heightsOf(ownSections.value, estimateHeight)
  return prefixTopsOf(heights, contentStart(), READER_TUNING.frameGap)
}
...
function enforceDomBudget(): void {
  ...
  const heights = heightsOf(ownSections.value, estimateHeight)
  const tops = prefixTopsOf(heights, contentStart(), READER_TUNING.frameGap)
...
function updateCurrentHeading(): void {
  ...
  if (readerState.pinnedTocId && readerState.pinnedTocHeading) {
    ...
    readerState.currentHeadingId = readerState.pinnedTocId
    return
  }
  const line = root.getBoundingClientRect().top + root.clientHeight * READER_TUNING.anchorLine
  const previous = precedingHeadings.value.get(id)
  let title = previous?.title ?? '',
    headingId = previous?.id ?? ''
  for (const h of frame.querySelectorAll<HTMLElement>(
    'h1[data-toc-id], h2[data-toc-id], h3[data-toc-id]'
  )) {
    if (h.getBoundingClientRect().top <= line) {
      title = headingText(h)
      headingId = h.dataset.tocId ?? ''
    } else break
  }
  readerState.currentHeadingTitle = title
  readerState.currentHeadingId = headingId
}
...
function onAnchorsReady(id: string, el: HTMLElement): void {
  ...
  anchors.forEach((a, i) => {
    ...
    if (!noteId) return
    a.setAttribute('aria-keyshortcuts', 'Enter Space Shift+Enter')
  ...
  if (id === readerState.currentSectionId) updateCurrentHeading()
}
...
function tocPinHeadingCrossed(): boolean {
  ...
  const hit = findHeadingEl(frame, heading, readerState.pinnedTocId)
  if (!hit) return false
...
function onReaderWheel(): void {
  jumpToken++ // Manual reading supersedes any pending asynchronous jump refinement.
  resumeFollow()
}
...
function onWindowKeydown(ev: KeyboardEvent): void {
  if (documentSession.mode !== 'read' || document.querySelector('dialog[open]')) return
  if (ev.target instanceof Element && ev.target.closest('.note-peek')) return
  if (
    ev.target instanceof HTMLElement &&
    ev.target.matches('.reader-scroll .zmu-ref') &&
    (ev.key === 'Enter' || ev.key === ' ')
  ) {
    ev.preventDefault()
    if (ev.shiftKey && ev.key === 'Enter') openNotePeek(ev.target.dataset.noteId!, ev.target, true)
    else void showNote(ev.target.dataset.noteId!)
    return
  }
...
function onReaderPointerDown(ev: PointerEvent): void {
  const root = scrollEl.value
  if (root && ev.target === root && ev.offsetX >= root.clientWidth) {
    jumpToken++
    resumeFollow()
  }
}
...
function onScroll(): void {
  if (documentSession.mode !== 'read') return
...
function doSaveProgress(force = false): void {
  if (documentSession.mode === 'edit' && !force) return
  if (!ownPath) return
...
async function waitForFineTuneTarget(
  ...
  for (let i = 0; i < READER_TUNING.fineTunePollFrames; i++) {
    ...
    if (frame && frame.querySelector('.section-body')) {
      ...
      if (heading) {
        const hit = findHeadingEl(frame, heading, headingId)
        if (hit) target = hit
      }
      lastTarget = target
      const top =
...
async function scrollToSection(
  ...
  if (
    (!source || !frameEls.get(id)?.querySelector('.section-body')) &&
    Math.abs(estTop - root.scrollTop) > root.clientHeight * 1.2
  ) {
  ...
  let target = await waitForFineTuneTarget(id, heading, token, headingId)
    ...
    target =
      frameEls
        ...
        ?.querySelector<HTMLElement>(
          '[data-source-block="' + CSS.escape(source.block.key) + '"]'
        ) ?? target
    if (!target.getClientRects().length && source.block.tag === 'h1')
      target = root.querySelector<HTMLElement>('.document-overture') ?? target
  }
  if (blockIndex !== undefined) {
    const body = frameEls.get(id)?.querySelector('.section-body')
    const block = body ? searchBlocks(body, Boolean(match))[blockIndex] : null
    if (block instanceof HTMLElement) {
      // The first H1 is represented by the visible overture. Its hidden duplicate
      // has a zero rectangle and cannot be used as a search or hash destination.
      target =
        block.tagName === 'H1' && !block.getClientRects().length
          ? (root.querySelector<HTMLElement>('.document-overture') ?? block)
          : block
      const highlight = target
      highlight.classList.add('search-landed')
      setTimeout(() => highlight.classList.remove('search-landed'), 3500)
      matchedRect = paintSearchMatch(
        target.classList.contains('document-overture')
          ? (target.querySelector('h1') ?? target)
          : target,
        match
      )
    }
  ...
  const top =
    root.scrollTop +
    (matchedRect?.()?.top ?? target.getBoundingClientRect().top) -
    root.getBoundingClientRect().top
  const intendedTop = target.classList.contains('document-overture')
    ? 0
    : Math.max(
        0,
        source
          ? top +
              target.getBoundingClientRect().height * source.ratio -
              root.clientHeight * source.viewport
          : top - 16
      )
  root.scrollTo({ top: intendedTop, behavior: source ? 'instant' : 'smooth' })
...
function headingText(h: Element): string {
  const text = h.cloneNode(true) as Element
  text.querySelectorAll('.zmu-ref').forEach((ref) => ref.remove())
  return text.textContent?.trim() ?? ''
}

/** 章容器内按标题文本精配小节元素（忽略全部空白差异） */
function findHeadingEl(frame: HTMLElement, title: string, headingId?: string): HTMLElement | null {
  if (headingId) {
    const exact = frame.querySelector<HTMLElement>(`[data-toc-id="${CSS.escape(headingId)}"]`)
    if (exact) {
      if (exact.tagName === 'H1' && !exact.getClientRects().length)
        return scrollEl.value?.querySelector<HTMLElement>('.document-overture') ?? exact
      return exact
    }
  }
...
async function onClick(ev: MouseEvent): Promise<void> {
  ...
  if (sup instanceof HTMLElement && sup.dataset.noteId) {
    await showNote(sup.dataset.noteId)
  }
}
...
watch(
  ...
  async (mode) => {
    ...
    const newBook = bookState.book
    if (!newBook) return
    if (ownBook.value !== newBook) {
    ...
    await nextTick()
    await nextFrames(2)
    const offset = currentDocumentOffset()
    if (offset !== undefined && documentPosition.value?.returnToReading) {
      documentPosition.value.returnToReading = false
      const place = sourcePlace(bookPositionIndex(newBook), offset)
      if (place?.block.kind === 'section')
        await scrollToSection(place.block.id, undefined, undefined, undefined, undefined, place)
      else if (place) {
        settings.sidebarVisible = true
        await nextTick()
        requestSidebarLocate(place.block.id, undefined, undefined, place)
      }
    }
  }
)

// Preserve the visible paragraph when typography or theme changes.
watch(
  () => [
    settings.fontSize,
    settings.lineHeight,
  ...
  async () => {
    const root = scrollEl.value
    if (!root || documentSession.mode !== 'read') return
    const atStart = root.scrollTop < 2
    const rootTop = root.getBoundingClientRect().top
    const target = [
      ...root.querySelectorAll<HTMLElement>('.section-body p, .section-body h2, .section-body h3')
    ].find((el) => el.getBoundingClientRect().bottom > rootTop + 24)
    const before = target?.getBoundingClientRect().top
    heightCache.clear()
    jumpToken++
    await nextTick()
    await nextFrames(2)
    if (atStart) root.scrollTop = 0
    else if (target?.isConnected && before !== undefined)
      root.scrollTop += target.getBoundingClientRect().top - before
  }
)
...
watch(
  ...
  () => {
    ...
    if (req.kind === 'section') {
      ...
      resumeFollow()
      void scrollToSection(req.id, req.heading, req.blockIndex, req.headingId, req.match)
    } else {
...
<template>
  <div ref="scrollEl" class="reader-scroll" @scroll.passive="onScroll" @click="onClick">
    ...
    >
      <DocumentOverture v-if="studio.themeId !== 'paper'" />
      <SectionFrame
      ...
      />
      <p v-if="studio.themeId !== 'paper'" class="reader-fin" aria-hidden="true">· 全书完 ·</p>
