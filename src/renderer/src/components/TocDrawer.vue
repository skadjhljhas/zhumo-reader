...
import { currentTheme, studio } from '../composables/useStudio'
import StudioIcon from './StudioIcon.vue'
import AppIcon from './AppIcon.vue'
import ChaoshengMemoryRail from './ChaoshengMemoryRail.vue'
...
const currentTocId = computed(() => {
  ...
  if (readerState.currentHeadingId) return readerState.currentHeadingId
...
<template>
  ...
  <Transition name="toc-slide">
    <aside v-if="uiState.tocOpen" class="toc-drawer" aria-label="目录">
      ...
      <button class="toc-panorama" @click="studio.panoramaOpen = true">
        <StudioIcon name="panorama" :size="16" /><span>展开全书长卷</span
      ...
      <ChaoshengMemoryRail v-if="['lucent', 'chaosheng'].includes(studio.themeId)" />
      <div v-else class="toc-footnote">
        <strong>{{ currentTheme.mood }}</strong
        ><span>阅读不必匆忙。<br />每一次停留，都在向深处。</span>
      </div>
    </aside>
  </Transition>
</template>
