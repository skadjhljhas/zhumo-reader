...
export function parseBook(source: string, options?: ParseOptions): ParsedBook {
  ...
  try {
    const normalized = options?.sourcePositions ? source.replace(/\r\n?/g, '\n') : source
    const bom = normalized.startsWith('\uFEFF') ? 1 : 0
    const protectedMath = protectMathSource(normalized.slice(bom))
    const pre = preprocess(protectedMath.source, { trackContent: options?.sourcePositions })
    ...
    const rendered = renderBook(
      ...
      options?.sourcePositions
        ? {
            body: pre.bodySources!,
            offset: bom,
            notes: new Map(
              pre.definitions.flatMap((definition) =>
                definition.source && definition.lineSources
                  ? [
                      [
                        definition.label,
                        {
                          lines: definition.lineSources,
                          from: definition.source.from,
                          to: definition.source.to
                        }
                      ] as const
                    ]
                  : []
    ...
    const notes: NoteRecord[] = graph.nodes.map((node) => {
      const record: NoteRecord = {
        ...
        ...(rendered.selectionRanges.get(node.label)
          ? { selectionRange: rendered.selectionRanges.get(node.label) }
          : {}),
    ...
    return {
      ...
      ...(rendered.sourceBlocks ? { sourceBlocks: rendered.sourceBlocks } : {}),
      stats: {
        chars: Math.max(0, Array.from(bodyText).length - rendered.selectionMetadataChars),
