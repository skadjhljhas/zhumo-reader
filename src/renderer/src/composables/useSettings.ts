...
import { applyFonts } from './importedFonts'
...
export const uiState = reactive({
  tocOpen: window.innerWidth >= 1280,
  settingsOpen: false
})
...
export async function persistSettingsNow(): Promise<void> {
  ...
  if (!loaded) return
  const job = saveQueue.then(() => window.api.saveSettings(snapshot))
...
function persist(): void {
  try {
    localStorage.setItem(LS_CACHE, JSON.stringify({ ...settings }))
  } catch {
    /* optional cache */
  }
  if (!loaded) return
