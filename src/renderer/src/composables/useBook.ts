...
import { isMockApi, previewSource } from '../dev/api-mock'
async function fallbackParse(source: string, levelCap: number): Promise<ParsedBook> {
  const { parseBook } = await import('../parser')
  return parseBook(source, { levelCap, sourcePositions: true })
}
import { settings } from './useSettings'
import { resetReaderState } from './readerStore'
import { documentSession, guardDocumentAction, resetDocumentSession } from './documentSession'
import { studio, loadBookPins } from './useStudio'
import { newNoteJourney } from './noteJourney'
import sampleSource from '../samples/reader-tour.md?raw'
import annotationExample from '../../../../docs/annotation-example.md?raw'
import { readDrafts, type Draft } from './drafts'
...
export function parseInWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    const abort = (): void => {
      ...
      finished = true
      cleanup()
      reject(new DOMException('已取消过期排版', 'AbortError'))
    }
    ...
    const fallback = (): void => {
      worker?.terminate()
      if (finished) return
      void fallbackParse(source, levelCap)
        .then(done)
        .catch((error: unknown) => {
          if (finished) return
          finished = true
    ...
    worker.postMessage({ source, options: { levelCap, sourcePositions: true } })
  })
}
...
async function loadPayload(payload: BookPayload, seq = ++loadSeq): Promise<void> {
  ...
  try {
    if (payload.epub) {
      const { projectEpub } = await import('../parser/epub-book')
      const projection = projectEpub(payload.epub)
      book = projection.book
      payload.content = projection.source
    } else book = await parseInWorker(payload.content, settings.noteLevelCap)
  } catch (err) {
    // parseBook 不抛错，此分支仅为防御
    if (payload.epub)
      throw new Error('EPUB 排版未完成：' + (err instanceof Error ? err.message : String(err)))
    book = await fallbackParse(payload.content, settings.noteLevelCap)
  ...
  // 看到的都是同一本书的完整三元组
  bookState.book = markRaw(payload.epub ? book : attachBookResources(book, payload.path))
  bookState.restore = restore
  bookState.payload = payload
  resetDocumentSession(payload.path, payload.content)
  void readDrafts(payload.path)
    .then((drafts) => {
      if (seq !== loadSeq) return
      if (payload.path.startsWith('zhumo:untitled:') && documentSession.mode === 'edit') return
      const pending = drafts.filter((draft) => draft.source !== payload.content)
      documentSession.recoveryAlternatives = pending
      documentSession.recovery = pending[0] ?? null
    })
    .catch(() => undefined)
  loadBookPins(payload.path)
  studio.notesTab = 'all'
  studio.explicitFollowPause = false
  studio.noteJourney = newNoteJourney()
  ...
  if (payload.epub?.warnings.length) bookState.statusMessage = payload.epub.warnings.join('；')
...
/** 系统对话框开书 */
export async function newMarkdown(draft?: Draft): Promise<void> {
  if (documentSession.saving || documentSession.source !== documentSession.savedSource) {
    guardDocumentAction(() => {
      void newMarkdown(draft)
    })
    return
  }
  const path = draft?.path ?? 'zhumo:untitled:' + crypto.randomUUID()
  await loadPayload({ path, title: '未命名', content: '' })
  if (bookState.payload?.path !== path) return
  if (draft) {
...
export async function openBookViaDialog(): Promise<void> {
  ...
  try {
    ...
    if (seq !== loadSeq) return
    if (payload) await loadPayload(payload, seq)
    else if (bookState.status === 'loading')
      bookState.status = bookState.book ? 'reading' : 'welcome'
  } catch (err) {
    if (seq !== loadSeq) return
    if (bookState.status === 'loading') bookState.status = bookState.book ? 'reading' : 'welcome'
...
export async function reparseCurrentBook(): Promise<void> {
  ...
  if (payload.epub) return
  const seq = ++loadSeq
  const book = await parseInWorker(payload.content, settings.noteLevelCap)
  if (seq !== loadSeq || bookState.payload?.path !== payload.path) return
  bookState.book = markRaw(attachBookResources(book, payload.path))
}

/** 回欢迎页（保留 payload 以便重开） */
export function backToWelcome(): void {
  if (documentSession.saving || documentSession.source !== documentSession.savedSource) {
    guardDocumentAction(backToWelcome)
    return
  }
  loadSeq++
  bookState.status = 'welcome'
...
export function openSampleBook(): void {
  guardDocumentAction(() => {
    void loadPayload({
      path: 'zhumo:reader-tour',
      title: '在文字旁边，留一个位置',
      content: previewSource('zhumo:reader-tour', sampleSource)
    })
  })
}
export function openAnnotationExample(): void {
  guardDocumentAction(() => {
    void loadPayload({
      path: 'zhumo:annotation-example',
      title: '一扇门的两种开放',
      content: annotationExample
    })
  })
}

/** Rebuild the reading projection after a committed edit, preserving the book identity. */
export async function applyDocumentSource(content: string): Promise<void> {
  if (!bookState.payload) return
  const path = bookState.payload.path
  const seq = ++loadSeq
  const book = await parseInWorker(content, settings.noteLevelCap)
  if (seq !== loadSeq || bookState.payload?.path !== path) return
  bookState.payload.content = content
  bookState.book = markRaw(attachBookResources(book, path))
}
