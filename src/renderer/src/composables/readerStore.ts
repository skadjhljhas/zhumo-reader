...
import type { SourcePlace } from './sourcePosition'
...
export function requestScroll(
  ...
  readerState.scrollRequest = {
    kind,
    id,
    heading,
    headingId,
    spot,
    seq: readerState.scrollRequest.seq + 1
  }
}

export function requestSearchLanding(
  sectionId: string,
  blockIndex: number,
  match?: SearchMatchLocation
): void {
...
/** 请求侧栏定位到某注释卡 */
export function requestSidebarLocate(
  noteId: string,
  blockIndex?: number,
  match?: SearchMatchLocation,
  source?: SourcePlace
): void {
  readerState.sidebarRequest = {
    noteId,
    blockIndex,
    match,
    source,
    seq: readerState.sidebarRequest.seq + 1
  }
}
