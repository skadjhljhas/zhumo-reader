...
const DEMO_BOOKS: Record<string, DemoBook> = {
  'syntax-meaning': {
    file: 'syntax-meaning-sample.md',
    path: '/demo/语言的光学.md',
    title: '语言的光学 · 固定案例'
  },
  'syntax-stream': {
    file: 'syntax-stream-sample.md',
    path: '/demo/句法逐批显影.md',
    title: '句法逐批显影 · 固定示意'
  },
...
const mock: ZhuMoApi = {
  async manuscriptLocation() {
    return { path: '', available: false, error: '文稿文件夹在桌面版中管理。' }
  },
  async chooseManuscriptLocation() {
    return null
  },
  async revealManuscriptLocation() {
    throw new Error('文稿文件夹在桌面版中打开。')
  },
  async saveBook() {
    throw new Error('浏览器预览不能覆盖本机文件，请使用「另存为」下载文档。')
  },
  async saveBookAs(name, content) {
    const blob = new Blob([content], { type: 'text/markdown;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `${name.replace(/\.(md|markdown|txt)$/i, '')}.md`
    a.click()
    setTimeout(() => URL.revokeObjectURL(url), 30000)
    // A browser download has no durable file handle; keep the draft unsaved.
    return null
  },
  async setDocumentDirty() {
    /* Browser beforeunload owns this state. */
  },
  async closeConfirmed() {
    /* Browser preview does not own the host window. */
  },
  ...
  async exportPdf() {
    throw new Error('请在朱墨桌面应用中导出 PDF。')
  },
  async importFont() {
    throw new Error('请在朱墨桌面应用中导入字体。')
  },
  async readFont() {
    throw new Error('请在朱墨桌面应用中读取字体。')
  },
...
/** Browser demonstration notes stay in this local preview; never claim a disk save. */
export function previewSource(path: string, original: string): string {
  if (!isMockApi) return original
  try {
    const entry = JSON.parse(localStorage.getItem('zhumo.preview-manuscript:' + path) ?? 'null')
    return entry?.original === original && typeof entry.source === 'string'
      ? entry.source
      : original
  } catch {
    return original
...
export function savePreviewSource(path: string, previous: string, source: string): void {
  if (!isMockApi) throw Error('仅供本机网页预览使用。')
  let original = previous
  try {
    const entry = JSON.parse(localStorage.getItem('zhumo.preview-manuscript:' + path) ?? 'null')
    if (entry?.source === previous) original = entry.original
  } catch {
    /* Use the current preview source. */
  }
  localStorage.setItem('zhumo.preview-manuscript:' + path, JSON.stringify({ original, source }))
}
