...
import { admitReader } from './update-admission'
import { runProfileGuard } from './profile-guard'
import { runVersionLauncher } from './version-launcher'
import { managedInstallRoot } from './managed-install'
import { startPerformanceReport } from './performance-report'
import { readFileSync } from 'node:fs'
import { productIdentity, allowsFileAssociations, type ProductIdentity } from './product-identity'
import {
  chooseNativeProfile,
...
function startReader(): void {
  ...
  try {
    ...
    if (!app.isPackaged && __CHAOSHENG_PREVIEW__) metadata.zhumoChannel = 'preview-theme'
    product = productIdentity(metadata)
    const helper = app.isPackaged
      ? join(
          dirname(app.getPath('exe')),
          'resources/app.asar.unpacked/out/main/native/ProfileChooser.exe'
        )
      : join(__dirname, 'native/ProfileChooser.exe')
    app.setPath(
      'userData',
      selectReaderProfile({
        appData: app.getPath('appData'),
        product,
        explicitProfile: process.env.ZHUMO_USER_DATA,
        choose:
          process.env.ZHUMO_TEST_BACKGROUND === '1'
            ? undefined
            : (choices) => chooseNativeProfile(helper, choices)
      })
    )
  } catch (error) {
    if (!(error instanceof ProfileSelectionCancelled) && process.env.ZHUMO_TEST_BACKGROUND !== '1')
      dialog.showErrorBox(
        '朱墨暂未打开',
        error instanceof Error ? error.message : '阅读资料位置不可用。'
      )
    app.exit(error instanceof ProfileSelectionCancelled ? 0 : 76)
    return
  }

  // Admit synchronously before ready/single-instance handling can open a profile. In a
  // blocked launch no renderer or user-data writer is initialized. Test coordination is isolated.
  let admitted = false
  let managedRoot: string | undefined
  try {
    managedRoot = managedInstallRoot(app.getPath('exe'), process.env.ZHUMO_INSTALL_ROOT)
    admitReader(
      [
      ...
      process.env.ZHUMO_UPDATE_COORDINATION ||
        join(app.getPath('appData'), 'ZhuMo-update-coordination')
    )
    admitted = true
  } catch (error) {
    const message = error instanceof Error ? error.message : '暂时无法确认更新状态。'
    if (!(process.env.ZHUMO_TEST_BACKGROUND === '1' && process.env.ZHUMO_USER_DATA))
      dialog.showErrorBox('朱墨暂未打开', message)
    console.error('[zhumo] ' + message)
    app.exit(73)
  }
  ...
  // Explicit automation mode: keep the real renderer running without presenting a
  // desktop window. A separate profile is required so tests cannot attach to a
  // reader the user already has open. Normal launches keep Electron's defaults.
  const backgroundTest =
    process.env.ZHUMO_TEST_BACKGROUND === '1' && Boolean(process.env.ZHUMO_USER_DATA)
  const sharedTextureTest = backgroundTest && process.env.ZHUMO_TEST_SHARED_TEXTURES === '1'
  const fileAssociationsAllowed = allowsFileAssociations(product, app.isPackaged, backgroundTest)
  ...
  function createWindow(bookPath?: string): BrowserWindow {
    const win = new BrowserWindow({
      ...
      title: product.title,
      show: false,
      ...(backgroundTest ? { focusable: false, skipTaskbar: true } : {}),
      ...
      webPreferences: {
        ...
        ...(backgroundTest
          ? { offscreen: sharedTextureTest ? { useSharedTexture: true } : true }
          : {})
      }
    })

    flushStates.set(win.id, { dirty: false, approved: false, pending: false, onAck: null })

    // A merely hidden native surface still receives sparse compositor frames on
    // Windows. Offscreen rendering supplies regular frames without a desktop
    // surface; its frame timings are not an on-screen performance benchmark.
    if (backgroundTest) win.webContents.setFrameRate(60)
    if (sharedTextureTest) win.webContents.on('paint', (event) => event.texture?.release())
    // Visible reading windows keep their display cadence when another window gains
    // focus. Minimized normal windows can still suspend their optical work.
    if (!backgroundTest) {
    ...
    win.on('ready-to-show', () => {
      if (!backgroundTest) win.show()
      void startPerformanceReport(win).catch((error) =>
        console.warn('[zhumo] 性能记录未启动：', error.message)
      )
    })
    ...
    win.on('close', (event) => {
      ...
      // Always ask the renderer: its latest keystroke may precede the dirty IPC message.
      // A cached clean flag is not permission to destroy an editing window.
      if (!state.approved) {
  ...
  ipcMain.on(IPC.DocumentDirty, (event, dirty: boolean) => {
    const win = BrowserWindow.fromWebContents(event.sender)
    const state = win ? flushStates.get(win.id) : undefined
    if (state && typeof dirty === 'boolean') state.dirty = dirty
  })
  ipcMain.handle(IPC.CloseConfirmed, (event) => {
    const win = BrowserWindow.fromWebContents(event.sender)
    const state = win ? flushStates.get(win.id) : undefined
    if (!win || !state) return
    state.dirty = false
  ...
  async function bootstrap(): Promise<void> {
    ...
    registerIpc(store, managedRoot, fileAssociationsAllowed)
    registerAiIpc()
    ...
    if (assocStore) {
      void ensureFileAssocStartup({
        packaged: fileAssociationsAllowed,
  ...
  const gotSingleInstanceLock = admitted && app.requestSingleInstanceLock()
...
if (process.argv.includes('--zhumo-launch-current')) {
  void runVersionLauncher().catch((error) => {
    const message = error instanceof Error ? error.message : '当前版本尚未通过启动校验。'
    if (process.env.ZHUMO_TEST_BACKGROUND !== '1') dialog.showErrorBox('朱墨暂未打开', message)
    console.error('[zhumo launcher] ' + message)
    app.exit(75)
  })
} else if (process.argv.includes('--zhumo-profile-guard')) {
  void runProfileGuard().catch((error) => {
    console.error('[zhumo guard] ' + (error instanceof Error ? error.message : '守护未完成。'))
    app.exit(74)
  })
} else startReader()
