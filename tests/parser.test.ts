...
describe('演示书 phg-sample', () => {
  ...
  it('notes 文档序（注内引用排在其父注之后）', () => {
    ...
    expect(book.sections[0].html).toContain(
      `<sup class="zmu-ref" data-note-id="${byLabel(book, '译者:确定性')?.id}" data-level="1" tabindex="0" role="button" aria-label="阅读注释 确定性"><span class="zmu-ref-mark">确定性</span></sup>`
    )
  })
  ...
  it('多引聚合一注：refCount / anchorSpots（含章标题）/ 双锚点', () => {
    ...
    expect(html).toContain(
      `<sup class="zmu-ref" data-note-id="${id}" data-level="1" tabindex="0" role="button" aria-label="阅读注释 确定性"><span class="zmu-ref-mark">确定性</span></sup>`
    )
  })

  it('多段落定义（缩进续行）', () => {
    const html = byLabel(book, '注1')?.html ?? ''
    expect(html).toContain('<p><span data-source-inline="0">译者注：种子之喻屡见于黑格尔著作')
    expect(html).toContain('<p><span data-source-inline="1">《精神现象学》序言中')
  ...
  it('KaTeX：正文行内/块级与注内公式', () => {
    ...
    expect(book.sections[0].html).toContain('zmu-math-block')
...
describe('数据契约', () => {
  ...
  it('标题净化：标题内的引用不进入 title，但锚点保留在渲染中', () => {
    ...
    expect(book3.sections[0].html).toContain(
      '<h1 data-toc-id="toc-1"><span data-source-inline="0">标题<sup'
    )
  })
})
...
describe('注释图', () => {
  ...
  it('notes 文档序（A/C/B/D 嵌套交错）', () => {
    ...
    expect(html).toContain(
      `data-note-id="${book.notes[0]?.id}" data-level="1" tabindex="0" role="button" aria-label="阅读注释 A"><span class="zmu-ref-mark">A</span></sup>`
    )
    expect(html).toContain(
      `data-note-id="${book.notes[2]?.id}" data-level="1" tabindex="0" role="button" aria-label="阅读注释 B"><span class="zmu-ref-mark">B</span></sup>`
    )
...
describe('归一与代码保护', () => {
  ...
  it('定义支持 4 空格缩进续行与多段落', () => {
    ...
    expect(html).toContain('<p><span data-source-inline="0">第一段。</span></p>')
    expect(html).toContain('<p><span data-source-inline="1">第二段续行。</span></p>')
...
describe('分章规则', () => {
  ...
  it('H2 → H3 二次瀑布', () => {
    ...
    expect(book.sections[0].html).toContain(
      '<h1 data-toc-id="toc-1"><span data-source-inline="0">章</span></h1>'
    )
...
describe('未闭合行内注抢断（M4）', () => {
  it('未闭合 ^[ 后跟代码块+标题：正文不丢、代码块按代码渲染', () => {
    ...
    // 标题与后续正文不丢
    expect(book.sections[0].html).toContain(
      '<h2 data-toc-id="toc-1"><span data-source-inline="1">后续标题</span></h2>'
    )
...
describe('环上 orderKey 缓存（M5）', () => {
  it('X↔Y 互引且 X 的正文引用晚于 Y：X 的 orderKey 符合首次引用路径', () => {
    ...
    const html = book.sections[0].html
    expect(html).toContain(
      `data-note-id="${byLabel(book, 'Y')?.id}" data-level="1" tabindex="0" role="button" aria-label="阅读注释 Y"><span class="zmu-ref-mark">Y</span></sup>`
    )
    expect(html).toContain(
      `data-note-id="${byLabel(book, 'X')?.id}" data-level="1" tabindex="0" role="button" aria-label="阅读注释 X"><span class="zmu-ref-mark">X</span></sup>`
    )
    expect(html).toContain(
      `data-note-id="${byLabel(book, 'Z')?.id}" data-level="1" tabindex="0" role="button" aria-label="阅读注释 Z"><span class="zmu-ref-mark">Z</span></sup>`
    )
  })
})
...
describe('T23：displayMark 作者标号', () => {
  ...
  it('标号写入 sup 并 HTML 转义（可访问名称与内层 mark 双通道）', () => {
    const book = parseBook('正文[^a<b]引。\n\n[^a<b]: 定义。')
    const html = book.sections[0].html
    expect(html).toContain('aria-label="阅读注释 a&lt;b"')
...
describe('T23：anchorSpots 引用地图上下文', () => {
  it('body spot 携带所在章标题（kind: body）', () => {
    ...
    expect(byLabel(book, 'n')?.anchorSpots).toEqual([
      { kind: 'body', sectionId: 'sec-1', order: 0, sectionTitle: '甲章', headingId: 'toc-1' }
    ])
  })
  ...
  it('嵌套注：注内引用产出 note spot（父注 id / 标号）', () => {
    ...
    expect(a?.anchorSpots).toEqual([
      { kind: 'body', sectionId: 'sec-1', order: 0, sectionTitle: '章', headingId: 'toc-1' }
    ])
  })
  ...
  it('多引混合：body 条目在前按 order 升序，note 条目在后', () => {
    ...
    expect(m?.anchorSpots).toEqual([
      { kind: 'body', sectionId: 'sec-1', order: 0, sectionTitle: '甲', headingId: 'toc-1' },
      { kind: 'body', sectionId: 'sec-1', order: 2, sectionTitle: '甲', headingId: 'toc-1' },
      { kind: 'body', sectionId: 'sec-2', order: 3, sectionTitle: '乙', headingId: 'toc-2' }
    ])
    expect(k?.anchorSpots).toEqual([
      { kind: 'body', sectionId: 'sec-1', order: 1, sectionTitle: '甲', headingId: 'toc-1' },
