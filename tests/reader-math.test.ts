 ...
 * - 侧栏跟随平滑插值（T24）：glideSpeedCap 巡航上限、glideFrame 临界阻尼弹簧
 *   一帧的不变式（单调逼近不过冲、帧率无关、有界落位、中途改目标不回摆、
 *   帧长异常不产生 NaN）与 nearestScrollDelta 的 nearest 等价位移。
 */
...
/* ================================================================== */
/* 侧栏跟随平滑插值（T24：instant 瞬移 → glide）                          */
/* ================================================================== */

/** NotesSidebar 的 rAF 循环所用的同一份调参（结构上即 GlideTuning） */
const glideTuning = {
...
describe('glide 调参与巡航上限（T24）', () => {
  it('新常量快照（只收敛不改值）', () => {
    expect(READER_TUNING.followGlideOmega).toBe(20)
    expect(READER_TUNING.followGlideMaxSpeed).toBe(4.5)
    expect(READER_TUNING.followGlideMaxMs).toBe(700)
    expect(READER_TUNING.followGlideSnapPx).toBe(0.5)
  })

  it('glideSpeedCap：近距取常数下限，超远按 距离/maxMs 抬高（有界落位）', () => {
    expect(glideSpeedCap(300, glideTuning)).toBe(glideTuning.maxSpeed)
    // 拐点：maxSpeed × maxMs 处两条口径相等
    expect(glideSpeedCap(glideTuning.maxSpeed * glideTuning.maxMs, glideTuning)).toBe(
      glideTuning.maxSpeed
    )
    expect(glideSpeedCap(10_000, glideTuning)).toBeCloseTo(10_000 / glideTuning.maxMs)
    // 上滚（负距离）同判：按绝对值
    expect(glideSpeedCap(-10_000, glideTuning)).toBeCloseTo(10_000 / glideTuning.maxMs)
  })
})

describe('nearestScrollDelta：scrollIntoView nearest 的等价位移（T24）', () => {
  it('卡顶越过视口上沿 → 上滚对齐视口顶（负位移）', () => {
    expect(nearestScrollDelta(-120, 80, 800)).toBe(-120)
    expect(nearestScrollDelta(-0.5, 200, 800)).toBe(-0.5)
  })

  it('卡底越过视口下沿 → 下滚对齐视口底（正位移）', () => {
    expect(nearestScrollDelta(900, 1100, 800)).toBe(300)
    expect(nearestScrollDelta(700, 801, 800)).toBe(1)
  })

  it('卡在视口内（含贴边、零高退化）→ 不动', () => {
    expect(nearestScrollDelta(0, 200, 800)).toBe(0)
    expect(nearestScrollDelta(700, 800, 800)).toBe(0)
    expect(nearestScrollDelta(300, 300, 800)).toBe(0)
  })

  it('跨视口的高卡（顶在上沿之上、底在下沿之下）优先对齐视口顶', () => {
    expect(nearestScrollDelta(-100, 900, 800)).toBe(-100)
  })
})

describe('glide 一帧插值 glideFrame（T24）', () => {
  /** 逐帧推进的不变式指标 */
  interface RunReport {
    position: number
    velocity: number
  ...
  /** 以固定帧长逐帧推进直至落位，收集不变式指标 */
  function run(from: number, to: number, dtMs = 16, maxFrames = 600): RunReport {
    ...
    for (let i = 1; i <= maxFrames; i++) {
      const f = glideFrame(position, velocity, to, dtMs, glideTuning)
      maxStep = Math.max(maxStep, Math.abs(f.position - position))
      if (dir * (f.position - position) < -1e-9) monotone = false
      if (dir * (f.position - to) > 1e-9) overshoot = Math.max(overshoot, Math.abs(f.position - to))
      if (ms95 < 0 && Math.abs(f.position - to) <= total * 0.05) ms95 = i * dtMs
      position = f.position
      velocity = f.velocity
      frames = i
      if (f.settled) {
  ...
  it('误差小于 snapPx 直接吸附（scrollTop 按像素量化，不留亚像素抖动）', () => {
    expect(glideFrame(299.6, 0, 300, 16, glideTuning)).toEqual({
      position: 300,
      velocity: 0,
      settled: true
    })
    expect(glideFrame(0.4, 0, 0, 16, glideTuning)).toEqual({
      position: 0,
      velocity: 0,
      settled: true
    })
  ...
  it('常规跟随距离：单调逼近、无过冲、95% 落位在 180–320ms 手感区间', () => {
    for (const dist of [40, 129, 240, 300, 800]) {
      const r = run(0, dist)
      expect(r.settled, `dist=${dist}`).toBe(true)
      expect(r.monotone, `dist=${dist}`).toBe(true)
      expect(r.overshoot, `dist=${dist}`).toBe(0)
      expect(r.ms95, `dist=${dist}`).toBeGreaterThanOrEqual(150)
      expect(r.ms95, `dist=${dist}`).toBeLessThanOrEqual(320)
      expect(r.position, `dist=${dist}`).toBe(dist)
      expect(r.velocity, `dist=${dist}`).toBe(0)
    }
  })

  it('上滚（负方向）同性质：单调、无过冲、精确落位', () => {
    const r = run(300, 0)
  ...
  it('单帧位移不超过巡航上限 × 帧长（消除基线 451px 级瞬移）', () => {
    for (const dist of [240, 300, 800, 2224]) {
      const r = run(0, dist)
      const budget = glideSpeedCap(dist, glideTuning) * 16
      expect(r.maxStep, `dist=${dist}`).toBeLessThanOrEqual(budget + 1)
      expect(r.maxStep, `dist=${dist}`).toBeLessThan(dist) // 首帧不瞬移到目标
    }
  })

  it('超远目标仍有界落位：不慢于「常数巡航速度」的线性下界，也不退化为爬行', () => {
    for (const dist of [3150, 5000, 20000]) {
      const r = run(0, dist)
      expect(r.settled, `dist=${dist}`).toBe(true)
      expect(r.monotone, `dist=${dist}`).toBe(true)
      expect(r.ms, `dist=${dist}`).toBeLessThanOrEqual(
        dist / glideTuning.maxSpeed + glideTuning.maxMs
      )
      expect(r.maxStep, `dist=${dist}`).toBeLessThanOrEqual(
        glideSpeedCap(dist, glideTuning) * 16 + 1
      )
    }
  })

  it('帧率无关：8/16/33/64ms 帧长下落位时间相近且单调（闭式积分而非逐帧累加）', () => {
    for (const dt of [8, 16, 33, 64]) {
      const r = run(0, 300, dt)
      expect(r.settled, `dt=${dt}`).toBe(true)
      expect(r.monotone, `dt=${dt}`).toBe(true)
      expect(r.ms, `dt=${dt}`).toBeLessThanOrEqual(600)
      expect(r.ms95, `dt=${dt}`).toBeLessThanOrEqual(320)
      expect(r.position, `dt=${dt}`).toBe(300)
    }
  })

  it('中途改目标（可打断、不叠加）：位置速度连续、折返不越过新目标', () => {
    // 飞向 400 的第 12 帧把目标改成 150 —— 模拟新 focus 卡打断在途动画
    let position = 0
    let velocity = 0
    let atRetarget = 0
    ...
    let settledFrame = -1
    for (let i = 1; i <= 200; i++) {
      const target = i < 12 ? 400 : 150
      if (i === 12) atRetarget = position
      const previous = position
      const f = glideFrame(position, velocity, target, 16, glideTuning)
      // 位置连续：改目标那一帧的位移仍在巡航预算内（不瞬移）
      if (i === 12)
        expect(Math.abs(f.position - previous)).toBeLessThanOrEqual(
          glideSpeedCap(400, glideTuning) * 16 + 1
        )
      position = f.position
      velocity = f.velocity
      if (i >= 12) minAfter = Math.min(minAfter, position)
      if (i >= 12 && f.settled) {
        settledFrame = i
        break
      }
    }
    expect(atRetarget).toBeGreaterThan(150) // 改目标时已飞过新目标，需要折返
    expect(settledFrame).toBeGreaterThan(0)
    expect(position).toBe(150)
    expect(minAfter).toBeGreaterThanOrEqual(150 - glideTuning.snapPx) // 折返不过冲
  })

  it('帧长异常（0/负/NaN）本帧不动且不产生 NaN；超大帧长按 64ms 截断', () => {
    for (const dt of [0, -50, Number.NaN]) {
      const f = glideFrame(0, 0, 300, dt, glideTuning)
      expect(Number.isFinite(f.position), `dt=${dt}`).toBe(true)
      expect(Number.isFinite(f.velocity), `dt=${dt}`).toBe(true)
      expect(f.position, `dt=${dt}`).toBe(0)
      expect(f.settled, `dt=${dt}`).toBe(false)
    }
    expect(glideFrame(0, 0, 300, 5000, glideTuning)).toEqual(glideFrame(0, 0, 300, 64, glideTuning))
    const clamped = glideFrame(0, 0, 300, 64, glideTuning)
    expect(clamped.position).toBeGreaterThan(0)
    expect(clamped.position).toBeLessThan(300)
  })
})
