...
describe('parseBookArg：生产形态', () => {
  ...
  it('相对路径解析为绝对路径', () => {
    expect(parseBookArg([EXE, 'fixtures\\phg-sample.md'])).toBe(resolve('fixtures\\phg-sample.md'))
  })
...
describe('parseBookArg：过滤规则', () => {
  it('过滤 electron 开关（--inspect / --remote-debugging-port 等）', () => {
    expect(
      parseBookArg([DEV_EXE, '--inspect', '--remote-debugging-port=9222', 'C:\\b\\real.md'])
    ).toBe('C:\\b\\real.md')
  })
  ...
  it('dev 形态：electron-vite 参数混入时仍能取到书', () => {
    ...
    expect(parseBookArg([DEV_EXE, '--trace-warnings', '.', 'C:\\b\\x.markdown'])).toBe(
      'C:\\b\\x.markdown'
    )
  })
})
